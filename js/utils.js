...
var save = function () {
	var date = new Date();
	if (canSave) {
		localStorage.setItem("SpaceL1", JSON.stringify(Game));
	}
	var tmp = new Date().getTime();
};

var load = function () {
	var savegame = JSON.parse(localStorage.getItem("SpaceL1"));
...
var restoreSave = function (save) {
	try {
		...
		JSON.parse(decoded);
		if (decoded) {
			localStorage.setItem("SpaceL1", decoded);
			canSave = 0;
			location.reload();
		} else {
