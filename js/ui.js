var UpdateUI = function() {
  ...
  $("#EXT-TITLE").html(
    texts.items[Missions[Game.extId].type] +
      "<img class='ui avatar image' src='images/items/" +
      Missions[Game.extId].type +
      ".png'>"
  );
  $("#EXT-DESC").html("The drone extract " + fix(Game.extGain, 1));
  $("#EXT-DESC2").html("every " + Game.extTime + " seconds.");
  $("#HYPERDRIVE-TEXT").html(
    "Travel to another location actually cost " +
      fix(Game.TravelCost, 3) +
      "% of power"
  );
  $("#HYPERSPACE-TEXT").html(
    "Your actual maximum destination is " +
      texts.systemname[Game.UnlockedLocations]
  );
  $("#Galaxy-content").html(
    "<span class='bold'>GALAXY NUMBER</span><br><h1 class='type4 or'>" +
      Game.Galaxy +
      "</h1>"
  );
  $("#Galaxy-content2").html(
    "<span class='bold'>REQUIREMENTS<br></span>- You must reach reach <span class='rouge'>Gaia</span>.<br>- Have <i class='green dollar sign icon'></i><span class='vert'>" +
      fix(GetGalaxyPrice(), 2) +
      "</span> to travel to another galaxy."
  );
  $("#EXPLO-TITLE").html("Exploration - " + texts.systemname[Game.system]);
  $("#galaxydesc").html(
    "There is a black hole here. It seems to be able to absorb everything, including my past, is it time to start all over again?"
  );
  $("#InventoryTitle").html(
    "Inventory <span class='type3'>(<span class='" +
      SetColor(Game.CurrInv) +
      "'>" +
      Game.CurrInv +
      "</span>/" +
      Game.Maxinv +
...
function GenMissions() {
  ...
  for (var i in Missions) {
    ...
    name =
      "<font class='text type1'>" +
      texts.systemname[offer.system] +
      "-" +
      offer.name +
      "</font>";
    if (Game.ExplorationMult[offer.type] < 0.9) {
      pricecolor = "vert";
    }
    if (Game.ExplorationMult[offer.type] > 1) {
      if (Game.ExplorationMult[offer.type] <= 2) {
        pricecolor = "rouge";
      }
    }
    if (Game.ExplorationMult[offer.type] > 0.9) {
    ...
    var SYSTEMDIV = $(
      "<tr class=''>" +
        ...
        ");'>" +
        exploretext +
        "</button><button class='ui " +
        canbuy10 +
        " button' onClick='explore(" +
        i +
        ", 10, " +
        offer.type +
        ");'>10</button><button class='ui " +
        canbuy100 +
        " button' onClick='explore(" +
        i +
        ", 100, " +
